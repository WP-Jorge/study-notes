...
3、容器相关属性：
    ...
    (4) justify-content：定义元素在主轴方向上的排列方式
        ...
        ② flex-end：从后往前排列
        ③ center：居中排列
        ④ space-between：分散排列，容器两边没有空隙
        ⑤ space-around：分散排列，容器两边有空隙
		⑥ space-evenly：分散排列，每个元素之间的空隙相同
    ...
    (6) align-content：定义多根轴线的对齐方式，如果项目只有一根轴线，那么该轴线将不起作用
        ...
        ⑤ space-between：轴线两端对齐，容器两边没有空隙
        ⑥ space-around：轴线两端对齐，边缘有间隙
		⑦ space-evenly：轴线两端对齐，每个元素之间的空隙相同
4、子元素相关属性：flex 元素属性：作用于 flex-box 内的子元素
    (1) order：默认值0，定义元素在容器中的排列顺序，数值越小，排列越靠前
    (2) flex-grow：默认值 0，定义元素的放大比例，即使有剩余空间，也不会放大
    (3) flex-shrink: 默认值 1，空间不足时将元素缩小，负数无效
	(4) flex-basis：定义了在分配多余空间之前，项目占的主轴空间，浏览器根据这个特性，计算主轴是由有多余空间，
        当主轴方向为 x 轴时，当设置了 flex-basis，项目的宽度将会失效，flex-basis 需要跟 flex-grow、flex-shrink 配合才能发挥效果
        ① 默认值 auto，元素本来的大小
...
- 给子元素设置 flex:1 是什么含义？flex 是哪几个属性的简写？flex-shrink 默认值是几？剩余空间的分配规则是怎样的？
...
```text
1、em 是相对长度单位，相对与当前对象文本的字体尺寸。如当前行内文本的字体尺寸未设置，则相对于浏览器的默认字体尺寸，
...
2、属性：
    ...
    (4) absolute：绝对定位，使用绝对定位的元素会脱离文档流，绝对定位的元素相对于自己最近的已定位的祖先元素，如果元素没有已定位的祖先元素，
...
- css 的 display 属性的三个属性值 block,inline 和 inline-block 有什么区别？
...
- 除了 flex 做响应式布局，还有别的方案吗 教程 https://juejin.cn/post/6844903814332432397 ）
...
3、元素设置了 float 浮动，解决方法：去除浮动
4、父元素的 z-index 比自己的 z-index 小，解决方法：提高父元素 z-index
```
...
- 实现一个宽高自适应的正方形
- css 实现垂直水平居中，尽可能多的方案
- 三栏布局以及怎么实现三列等高
- 实现动画，元素先向右移动 200px，再返回原点，一共移动 n 次（animation 添加两个属性即可）
