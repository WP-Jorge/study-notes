...
```text
1、commonJS 导入是同步的，在导入时会阻塞下面的代码执行，输出的是对输出值的浅拷贝，运行时加载
...
```text
1、set：成员唯一，不重复，是一组key的集合，可以遍历，保留插入顺序
2、map：以键值对的形式存储数据，具有极快的查找速度，允许任意类型的 key（包括NaN），可以遍历，保留插入顺序
```
...
2、object：键名只能为基本数据类型，属性顺序随机，不能通过 for of 访问，无法直接获取对象的长度
```
...
```js
Number.MAX_VALUE = 2^53 - 1; // 9007199254740991 
Number.MIN_VALUE = -(2^53 - 1); // -9007199254740991
```
...
```text
1、useCapture：默认值 false。指定事件处理函数的时期或阶段如果是 true 表示在事件捕获阶段执行，false 表示在事件冒泡阶段执行
...
3、sessionStorage：
    ...
    (2) sessionStorage 存储空间为 5MB，数据仅在当前会话中有效，如果浏览器标签或者页面关闭，则会删除在 sessionStorage 中的数据，在同一个标签中打开的新标签会复制顶层标签的 sessionStorage
...
1、EventLoop：
    ...
    (2) 执行一个宏任务，过程中遇到微任务时，将其放到微任务的事件队列里，当前宏任务执行完成后，会查看微任务的事件队列，依次执行里面的微任务。
	(3) 如果还有宏任务的话，再重新开启宏任务
2、微任务:
...
- 连续赋值
```
运算符优先级: 圆括号>成员属性>后置递增递减>逻辑非>前置递增、typeof>逻辑与/或>赋值
```
...
// function a
// 立即调用的函数表达式（IIFE）有一个自己独立的作用域，如果函数名称与内部变量名称冲突，就会永远优先执行函数本身
```
