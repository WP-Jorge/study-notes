package app.xiaofeiqiandaoguanli.xiaofeiqiandao;
...
public class Xiaofeiqiandao extends BaseFormController {
	// 业务处理方法
	public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (flag != null && flag.equals("getJsonStore")) {
				...
				String graphURL = doChart(sqlMap, request, response);
				resultMap.put("graphURL", graphURL);
				return toFinish("/WEB-ROOT/app/xiaofeiqiandaoguanli/xiaofeiqiandao/chart.jsp", resultMap, request, response);
			} else {
				HashMap<Object, Object> resultMap = new HashMap<Object, Object>();
				return toFinish("/WEB-ROOT/app/xiaofeiqiandaoguanli/xiaofeiqiandao/xiaofeiqiandao.jsp", resultMap, request, response);
			}
			// 提交数据库
	...
	public ModelAndView doGetJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String huiyuanzhanghaoSearch = request.getParameter("huiyuanzhanghaoSearch");
			if (huiyuanzhanghaoSearch != null && !huiyuanzhanghaoSearch.equals("")) {
				where.put("huiyuanzhanghaoSearch", huiyuanzhanghaoSearch);
			}
			String xiaofeiriqiSearch = request.getParameter("xiaofeiriqiSearch");
			if (xiaofeiriqiSearch != null && !xiaofeiriqiSearch.equals("")) {
				where.put("xiaofeiriqiSearch", xiaofeiriqiSearch);
			}
			...
			// 根据查询条件获取数据
			String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Xiaofeiqiandao.selecteList");
	...
	public ModelAndView doAdd(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			String huiyuanzhanghao = request.getParameter("huiyuanzhanghaoHidd");
			where.put("huiyuanzhanghao", huiyuanzhanghao);
			String xiaofeiriqi = request.getParameter("xiaofeiriqi");
			where.put("xiaofeiriqi", xiaofeiriqi);
			String shouliren = request.getParameter("shouliren");
			where.put("shouliren", shouliren);
			String shoulishijian = request.getParameter("shoulishijian");
			where.put("shoulishijian", shoulishijian);
			...
			map.put("yonghuzhanghaoSearch", huiyuanzhanghao);
			map.put("deleteFlagSearch", 0);
			List list = IbatisUtil.queryForList(sqlMap, request, response, map, "Zhucehuiyuan.selecteList");
			if( list != null && list.size() > 0 ){
				String huiyuanleixing = ((HashMap)(list.get(0))).get("huiyuanleixing").toString();
				if( huiyuanleixing.equals("次卡") ){
					String shengyucishu = ((HashMap)(list.get(0))).get("shengyucishu").toString();
					if( shengyucishu.equals("0") ){
						throw new Exception("剩余次数不足。请充值！");
					}else{
						HashMap m = new HashMap();
						m.put("yonghuzhanghao", huiyuanzhanghao);
						sqlMap.update("Zhucehuiyuan.updateObj1", m);
					}
				}else{
					String daoqishijian = ((HashMap)(list.get(0))).get("daoqishijian").toString();
					String nowDate = DateUtil.parse(new Date(), "yyyy-MM-dd");
					if( DateUtil.daysBetween( daoqishijian , nowDate  ) > 0 ){
						throw new Exception("会员卡到期。请充值！");
					}
				}
			}
			...
			// 向数据库新增数据
			sqlMap.insert("Xiaofeiqiandao.insertObj", where);
	...
	public ModelAndView doAddWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			// 新增数据
			sqlMap.insert("Xiaofeiqiandao.insertObj", whereMap);
	...
	public ModelAndView doUpdate(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			// 向数据库提交修改数据
			sqlMap.update("Xiaofeiqiandao.updateObj", where);
	...
	public ModelAndView doUpdateWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			// 向数据库提交修改信息
			sqlMap.update("Xiaofeiqiandao.updateObj", whereMap);
	...
	public ModelAndView doDeleOrUnDele(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			// 向数据库提交删除数据
			sqlMap.delete("Xiaofeiqiandao.doDeleOrUnDele", where);
	...
	public ModelAndView doShenPi(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			// 向数据库提交审批信息
			sqlMap.delete("Xiaofeiqiandao.doShenPi", where);
	...
	public static ModelAndView doExportExcel(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			// 根据查询条件得到结果集
			HashMap<String, Object> where = new HashMap<String, Object>();
			List<Object> listInfo = sqlMap.queryForList("Xiaofeiqiandao.selecteList", where);
	...
	public static WritableSheet refreashSheet(List<Object> al, WritableSheet ws) throws Exception {
		...
		// 设置表头(列号,行号 ,内容 )
		Label label_0 = new Label(1, 3, "会员帐号", wcfFC5);
		ws.addCell(label_0);
		Label label_1 = new Label(2, 3, "消费日期", wcfFC5);
		ws.addCell(label_1);
		Label label_2 = new Label(3, 3, "受理人", wcfFC5);
		ws.addCell(label_2);
		Label label_3 = new Label(4, 3, "受理时间", wcfFC5);
		ws.addCell(label_3);
		Label label_4 = new Label(5, 3, "Id", wcfFC5);
		ws.addCell(label_4);
		Label label_5 = new Label(6, 3, "创建时间", wcfFC5);
		ws.addCell(label_5);
		Label label_6 = new Label(7, 3, "备注", wcfFC5);
		ws.addCell(label_6);

		// 设置表体
		for (int i = 0; i < al.size(); i++) {
			String huiyuanzhanghao = ((HashMap) (al.get(i))).get("huiyuanzhanghao").toString();
			label_0 = new Label(1, i + 4, huiyuanzhanghao, wcfF4);
			ws.addCell(label_0);
			String xiaofeiriqi = ((HashMap) (al.get(i))).get("xiaofeiriqi").toString();
			label_1 = new Label(2, i + 4, xiaofeiriqi, wcfF4);
			ws.addCell(label_1);
			String shouliren = ((HashMap) (al.get(i))).get("shouliren").toString();
			label_2 = new Label(3, i + 4, shouliren, wcfF4);
			ws.addCell(label_2);
			String shoulishijian = ((HashMap) (al.get(i))).get("shoulishijian").toString();
			label_3 = new Label(4, i + 4, shoulishijian, wcfF4);
			ws.addCell(label_3);
	...
	public ModelAndView doAddPicSubmit(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			// 查询图片信息
			List list = sqlMap.queryForList("Xiaofeiqiandao.getTuPianIndex", whereMap);
			if (list != null && list.size() > 0) {
				if (list.get(0) != null) {
			...
			if (tuPianIndex.equals("")) {
				...
				sqlMap.update("Xiaofeiqiandao.updateTuPianIndex", whereMap);
			...
			while (iter.hasNext()) {
				...
				if (!item.isFormField()) {
					...
					if (!new File(uploadPath).exists()) {
						whereMap.put("operatorId", SysInfo.getLoginUserId(request, response));
						whereMap.put("tuPianIndex", tuPianIndex);
						sqlMap.insert("Xiaofeiqiandao.insertTuPian", whereMap);
					}
	...
	public ModelAndView doGetPicJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			// 得到查询图片结果信息
			String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Xiaofeiqiandao.selectePicList");
	...
	public ModelAndView doDelePic(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			// 向数据库提交删除信息
			sqlMap.delete("Xiaofeiqiandao.doDelePic", where);
	...
	public String doChart(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
		String filename = "";
		...
		try {
			...
			List yongHuXuanXiangList = sqlMap.queryForList("Xiaofeiqiandao.dochart", where);

			DefaultPieDataset dataset = new DefaultPieDataset();

			if( yongHuXuanXiangList != null && yongHuXuanXiangList.size() > 0 ){
				for( int j = 0 ; j < yongHuXuanXiangList.size() ; j ++ ){
					String pinglunshijian = ((HashMap)(yongHuXuanXiangList.get(j))).get("shijian").toString();
					String pinglungeshu = ((HashMap)(yongHuXuanXiangList.get(j))).get("cishu").toString();
					dataset.setValue(pinglunshijian, Integer.parseInt(pinglungeshu));
				}
			}
			...
			//加个副标题  
			chart.addSubtitle(new TextTitle("数据显示"));  
