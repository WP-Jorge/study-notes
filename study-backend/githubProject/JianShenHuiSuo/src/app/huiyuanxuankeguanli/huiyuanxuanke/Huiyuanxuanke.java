package  app.huiyuanxuankeguanli.huiyuanxuanke;
...
public class Huiyuanxuanke extends BaseFormController {
    //业务处理方法
    public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            if (flag != null && flag.equals("getJsonStore")) {
                ...
                return toFinish("/WEB-ROOT/app/huiyuanxuankeguanli/huiyuanxuanke/huiyuanxuanke.jsp", resultMap, request,  response);
            }
            //提交数据库
...
String kechengmingchenSearch = request.getParameter("kechengmingchenSearch"); 
 if( kechengmingchenSearch != null && !kechengmingchenSearch.equals("") ){  
 	 where.put("kechengmingchenSearch", kechengmingchenSearch);  
 } 
String xuandingriqiSearch = request.getParameter("xuandingriqiSearch"); 
 if( xuandingriqiSearch != null && !xuandingriqiSearch.equals("") ){  
 	 where.put("xuandingriqiSearch", xuandingriqiSearch);  
 } 
...
if( !SysInfo.getLoginUserType(request, response).equals("1") ){ 
	   where.put("dataRight",  "  a.operatorId='"+SysInfo.getLoginUserId(request, response)+"'  " );
            ...
            //根据查询条件获取数据
            String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Huiyuanxuanke.selecteList");
    ...
    public ModelAndView doAdd(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            String huiyuanzhanghao = request.getParameter("huiyuanzhanghao"); 
where.put("huiyuanzhanghao", huiyuanzhanghao);  
String kechengmingchen = request.getParameter("kechengmingchenHidd"); 
where.put("kechengmingchen", kechengmingchen);  
String xuandingriqi = request.getParameter("xuandingriqi"); 
where.put("xuandingriqi", xuandingriqi);  
...
where.put("erjiguanlianzd", erjiguanlianzd);  
            ...
            //向数据库新增数据
            sqlMap.insert("Huiyuanxuanke.insertObj", where);
    ...
    public ModelAndView doAddWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
            ...
            //新增数据
		    sqlMap.insert("Huiyuanxuanke.insertObj", whereMap);
...
where.put("erjiguanlianzd", erjiguanlianzd);  
            ...
            //向数据库提交修改数据
            sqlMap.update("Huiyuanxuanke.updateObj", where);
    ...
    public ModelAndView doUpdateWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//向数据库提交修改信息
		    sqlMap.update("Huiyuanxuanke.updateObj", whereMap);
    ...
    public ModelAndView doDeleOrUnDele(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交删除数据
            sqlMap.delete("Huiyuanxuanke.doDeleOrUnDele", where);
     ...
     public ModelAndView doShenPi(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交审批信息
            sqlMap.delete("Huiyuanxuanke.doShenPi", where);
...
if( !SysInfo.getLoginUserType(request, response).equals("1") ){ 
    ...
    public static ModelAndView doExportExcel(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			// 根据查询条件得到结果集
			HashMap<String, Object> where = new HashMap<String, Object>();
			List<Object> listInfo = sqlMap.queryForList("Huiyuanxuanke.selecteList", where);
 ...
 ws.addCell(label_0); 
 Label label_1 = new Label(2, 3, "课程名称", wcfFC5);  
 ws.addCell(label_1); 
 Label label_2 = new Label(3, 3, "选定日期", wcfFC5);  
 ws.addCell(label_2); 
 ...
 ws.addCell(label_5); 
		...
		//设置表体
		for (int i = 0; i < al.size(); i++) { String huiyuanzhanghao = ((HashMap) (al.get(i))).get("huiyuanzhanghao").toString();  
 label_0 = new Label(1, i + 4, huiyuanzhanghao, wcfF4); 
 ws.addCell(label_0); 
 String kechengmingchen = ((HashMap) (al.get(i))).get("kechengmingchen").toString();  
 label_1 = new Label(2, i + 4, kechengmingchen, wcfF4); 
 ws.addCell(label_1); 
 String xuandingriqi = ((HashMap) (al.get(i))).get("xuandingriqi").toString();  
 label_2 = new Label(3, i + 4, xuandingriqi, wcfF4); 
 ws.addCell(label_2); 
 ...
 ws.addCell(label_5); 
	...
	public ModelAndView doAddPicSubmit(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//查询图片信息
			List list = sqlMap.queryForList("Huiyuanxuanke.getTuPianIndex", whereMap);
			if( list != null && list.size() > 0 ){
				if( list.get(0) != null ){
			...
			if( tuPianIndex.equals("") ){
				...
				sqlMap.update("Huiyuanxuanke.updateTuPianIndex", whereMap);
				...
				if ( !item.isFormField()) {
				   //获取文件名称
					...
					if( ! new File( uploadPath ).exists() ){
						whereMap.put("operatorId", SysInfo.getLoginUserId(request, response));
						whereMap.put("tuPianIndex", tuPianIndex);
						sqlMap.insert("Huiyuanxuanke.insertTuPian", whereMap);
					}
	...
	public ModelAndView doGetPicJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//得到查询图片结果信息
			String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Huiyuanxuanke.selectePicList");
	...
	public ModelAndView doDelePic(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ...
	    try {
		    ...
		    //向数据库提交删除信息
		    sqlMap.delete("Huiyuanxuanke.doDelePic", where);
