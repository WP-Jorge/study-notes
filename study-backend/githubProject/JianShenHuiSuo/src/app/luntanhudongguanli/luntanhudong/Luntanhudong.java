package  app.luntanhudongguanli.luntanhudong;
...
public class Luntanhudong extends BaseFormController {
    //业务处理方法
    public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            if (flag != null && flag.equals("getJsonStore")) {
                ...
                return toFinish("/WEB-ROOT/app/luntanhudongguanli/luntanhudong/luntanhudong.jsp", resultMap, request,  response);
            }
            //提交数据库
    ...
    public ModelAndView doGetJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            String fabiaotimuSearch = request.getParameter("fabiaotimuSearch"); 
 if( fabiaotimuSearch != null && !fabiaotimuSearch.equals("") ){  
 	 where.put("fabiaotimuSearch", fabiaotimuSearch);  
 } 
String fabiaoneirongSearch = request.getParameter("fabiaoneirongSearch"); 
 if( fabiaoneirongSearch != null && !fabiaoneirongSearch.equals("") ){  
 	 where.put("fabiaoneirongSearch", fabiaoneirongSearch);  
 } 
...
if( !SysInfo.getLoginUserType(request, response).equals("1") ){ 
	   where.put("dataRight",  "  a.operatorId='"+SysInfo.getLoginUserId(request, response)+"'  " );
            ...
            //根据查询条件获取数据
            String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Luntanhudong.selecteList");
    ...
    public ModelAndView doAdd(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库新增数据
            sqlMap.insert("Luntanhudong.insertObj", where);
    ...
    public ModelAndView doAddWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if (item.isFormField()) {
					if (i == 0) {
	whereMap.put("fabiaotimu", item.getString("gb2312"));
}
else if (i == 1) {
	whereMap.put("fabiaoneirong", item.getString("gb2312"));
}
else if (i == 2) {
	whereMap.put("faburen", item.getString("gb2312"));
}
else if (i == 3) {
	whereMap.put("fabushijian", item.getString("gb2312"));
}
else if (i == 4) {
 ...
 whereMap.put("erjiguanlianzd",  request.getParameter("erjiguanlianzd")  );
            ...
            //新增数据
		    sqlMap.insert("Luntanhudong.insertObj", whereMap);
    ...
    public ModelAndView doUpdate(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交修改数据
            sqlMap.update("Luntanhudong.updateObj", where);
	...
	whereMap.put("detail", item.getString("gb2312"));
			...
			//向数据库提交修改信息
		    sqlMap.update("Luntanhudong.updateObj", whereMap);
    ...
    public ModelAndView doDeleOrUnDele(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交删除数据
            sqlMap.delete("Luntanhudong.doDeleOrUnDele", where);
     ...
     public ModelAndView doShenPi(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交审批信息
            sqlMap.delete("Luntanhudong.doShenPi", where);
...
if( !SysInfo.getLoginUserType(request, response).equals("1") ){ 
    ...
    public static ModelAndView doExportExcel(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			// 根据查询条件得到结果集
			HashMap<String, Object> where = new HashMap<String, Object>();
			List<Object> listInfo = sqlMap.queryForList("Luntanhudong.selecteList", where);
	...
	public static WritableSheet refreashSheet(List<Object> al, WritableSheet ws) throws Exception {
       ...
       //样式5
		...
		// 设置表头(列号,行号 ,内容 )
		 Label label_0 = new Label(1, 3, "发表题目", wcfFC5);  
 ws.addCell(label_0); 
 Label label_1 = new Label(2, 3, "发表内容", wcfFC5);  
 ws.addCell(label_1); 
 Label label_2 = new Label(3, 3, "发布人", wcfFC5);  
 ws.addCell(label_2); 
 Label label_3 = new Label(4, 3, "发布时间", wcfFC5);  
 ws.addCell(label_3); 
 ...
 ws.addCell(label_6); 
		...
		//设置表体
		for (int i = 0; i < al.size(); i++) { String fabiaotimu = ((HashMap) (al.get(i))).get("fabiaotimu").toString();  
 label_0 = new Label(1, i + 4, fabiaotimu, wcfF4); 
 ws.addCell(label_0); 
 String fabiaoneirong = ((HashMap) (al.get(i))).get("fabiaoneirong").toString();  
 label_1 = new Label(2, i + 4, fabiaoneirong, wcfF4); 
 ws.addCell(label_1); 
 String faburen = ((HashMap) (al.get(i))).get("faburen").toString();  
 label_2 = new Label(3, i + 4, faburen, wcfF4); 
 ws.addCell(label_2); 
 String fabushijian = ((HashMap) (al.get(i))).get("fabushijian").toString();  
 label_3 = new Label(4, i + 4, fabushijian, wcfF4); 
 ws.addCell(label_3); 
 ...
 ws.addCell(label_6); 
	...
	public ModelAndView doAddPicSubmit(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//查询图片信息
			List list = sqlMap.queryForList("Luntanhudong.getTuPianIndex", whereMap);
			if( list != null && list.size() > 0 ){
				if( list.get(0) != null ){
			...
			if( tuPianIndex.equals("") ){
				...
				sqlMap.update("Luntanhudong.updateTuPianIndex", whereMap);
				...
				if ( !item.isFormField()) {
				   //获取文件名称
					...
					if( ! new File( uploadPath ).exists() ){
						whereMap.put("operatorId", SysInfo.getLoginUserId(request, response));
						whereMap.put("tuPianIndex", tuPianIndex);
						sqlMap.insert("Luntanhudong.insertTuPian", whereMap);
					}
	...
	public ModelAndView doGetPicJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//得到查询图片结果信息
			String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Luntanhudong.selectePicList");
	...
	public ModelAndView doDelePic(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ...
	    try {
		    ...
		    //向数据库提交删除信息
		    sqlMap.delete("Luntanhudong.doDelePic", where);
