package  app.jiaolianxinxiguanli.jiaolianxinxi;
...
public class Jiaolianxinxi extends BaseFormController {
    //业务处理方法
    public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            if (flag != null && flag.equals("getJsonStore")) {
                ...
                return toFinish("/WEB-ROOT/app/jiaolianxinxiguanli/jiaolianxinxi/jiaolianxinxi.jsp", resultMap, request,  response);
            }
            //提交数据库
...
String chushengriqiSearch = request.getParameter("chushengriqiSearch"); 
 if( chushengriqiSearch != null && !chushengriqiSearch.equals("") ){  
 	 where.put("chushengriqiSearch", chushengriqiSearch);  
 } 
String ruxingshijianSearch = request.getParameter("ruxingshijianSearch"); 
 if( ruxingshijianSearch != null && !ruxingshijianSearch.equals("") ){  
 	 where.put("ruxingshijianSearch", ruxingshijianSearch);  
 } 
String congyejingliSearch = request.getParameter("congyejingliSearch"); 
 if( congyejingliSearch != null && !congyejingliSearch.equals("") ){  
 	 where.put("congyejingliSearch", congyejingliSearch);  
 } 
String gerenjianjieSearch = request.getParameter("gerenjianjieSearch"); 
 if( gerenjianjieSearch != null && !gerenjianjieSearch.equals("") ){  
 	 where.put("gerenjianjieSearch", gerenjianjieSearch);  
 } 
...
String erjiguanlianzdSearch = request.getParameter("erjiguanlianzdSearch"); 
 if( erjiguanlianzdSearch != null && !erjiguanlianzdSearch.equals("") ){  
 	 where.put("erjiguanlianzdSearch", erjiguanlianzdSearch);  
            ...
            //根据查询条件获取数据
            String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Jiaolianxinxi.selecteList");
    ...
    public ModelAndView doAdd(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库新增数据
            sqlMap.insert("Jiaolianxinxi.insertObj", where);
    ...
    public ModelAndView doAddWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if (item.isFormField()) {
					if (i == 0) {
	whereMap.put("jiaolianmingchen", item.getString("gb2312"));
}
else if (i == 1) {
	whereMap.put("xingbie", item.getString("gb2312"));
}
else if (i == 2) {
	whereMap.put("chushengriqi", item.getString("gb2312"));
}
else if (i == 3) {
	whereMap.put("ruxingshijian", item.getString("gb2312"));
}
else if (i == 4) {
	whereMap.put("congyejingli", item.getString("gb2312"));
}
else if (i == 5) {
	whereMap.put("gerenjianjie", item.getString("gb2312"));
}
else if (i == 6) {
	whereMap.put("lianxidianhua", item.getString("gb2312"));
}
else if (i == 7) {
 ...
 whereMap.put("erjiguanlianzd",  request.getParameter("erjiguanlianzd")  );
            ...
            //新增数据
		    sqlMap.insert("Jiaolianxinxi.insertObj", whereMap);
    ...
    public ModelAndView doUpdate(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交修改数据
            sqlMap.update("Jiaolianxinxi.updateObj", where);
	...
	whereMap.put("detail", item.getString("gb2312"));
			...
			//向数据库提交修改信息
		    sqlMap.update("Jiaolianxinxi.updateObj", whereMap);
    ...
    public ModelAndView doDeleOrUnDele(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交删除数据
            sqlMap.delete("Jiaolianxinxi.doDeleOrUnDele", where);
     ...
     public ModelAndView doShenPi(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交审批信息
            sqlMap.delete("Jiaolianxinxi.doShenPi", where);
    ...
    public static ModelAndView doExportExcel(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			// 根据查询条件得到结果集
			HashMap<String, Object> where = new HashMap<String, Object>();
			List<Object> listInfo = sqlMap.queryForList("Jiaolianxinxi.selecteList", where);
 ...
 ws.addCell(label_0); 
 Label label_1 = new Label(2, 3, "性别", wcfFC5);  
 ws.addCell(label_1); 
 Label label_2 = new Label(3, 3, "出生日期", wcfFC5);  
 ws.addCell(label_2); 
 Label label_3 = new Label(4, 3, "入行时间", wcfFC5);  
 ws.addCell(label_3); 
 Label label_4 = new Label(5, 3, "从业经历", wcfFC5);  
 ws.addCell(label_4); 
 Label label_5 = new Label(6, 3, "个人简介", wcfFC5);  
 ws.addCell(label_5); 
 Label label_6 = new Label(7, 3, "联系电话", wcfFC5);  
 ws.addCell(label_6); 
 Label label_7 = new Label(8, 3, "Id", wcfFC5);  
 ws.addCell(label_7); 
 Label label_8 = new Label(9, 3, "创建时间", wcfFC5);  
 ws.addCell(label_8); 
 Label label_9 = new Label(10, 3, "备注", wcfFC5);  
 ws.addCell(label_9); 
 ...
 label_1 = new Label(2, i + 4, xingbie, wcfF4); 
 ws.addCell(label_1); 
 String chushengriqi = ((HashMap) (al.get(i))).get("chushengriqi").toString();  
 label_2 = new Label(3, i + 4, chushengriqi, wcfF4); 
 ws.addCell(label_2); 
 String ruxingshijian = ((HashMap) (al.get(i))).get("ruxingshijian").toString();  
 label_3 = new Label(4, i + 4, ruxingshijian, wcfF4); 
 ws.addCell(label_3); 
 String congyejingli = ((HashMap) (al.get(i))).get("congyejingli").toString();  
 label_4 = new Label(5, i + 4, congyejingli, wcfF4); 
 ws.addCell(label_4); 
 String gerenjianjie = ((HashMap) (al.get(i))).get("gerenjianjie").toString();  
 label_5 = new Label(6, i + 4, gerenjianjie, wcfF4); 
 ws.addCell(label_5); 
 String lianxidianhua = ((HashMap) (al.get(i))).get("lianxidianhua").toString();  
 label_6 = new Label(7, i + 4, lianxidianhua, wcfF4); 
 ws.addCell(label_6); 
 String id = ((HashMap) (al.get(i))).get("id").toString();  
 label_7 = new Label(8, i + 4, id, wcfF4); 
 ws.addCell(label_7); 
 String itime = ((HashMap) (al.get(i))).get("itime").toString();  
 label_8 = new Label(9, i + 4, itime, wcfF4); 
 ws.addCell(label_8); 
 String detail = ((HashMap) (al.get(i))).get("detail").toString();  
 label_9 = new Label(10, i + 4, detail, wcfF4); 
 ws.addCell(label_9); 
 } 
	...
	public ModelAndView doAddPicSubmit(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//查询图片信息
			List list = sqlMap.queryForList("Jiaolianxinxi.getTuPianIndex", whereMap);
			if( list != null && list.size() > 0 ){
				if( list.get(0) != null ){
			...
			if( tuPianIndex.equals("") ){
				...
				sqlMap.update("Jiaolianxinxi.updateTuPianIndex", whereMap);
				...
				if ( !item.isFormField()) {
				   //获取文件名称
					...
					if( ! new File( uploadPath ).exists() ){
						whereMap.put("operatorId", SysInfo.getLoginUserId(request, response));
						whereMap.put("tuPianIndex", tuPianIndex);
						sqlMap.insert("Jiaolianxinxi.insertTuPian", whereMap);
					}
	...
	public ModelAndView doGetPicJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//得到查询图片结果信息
			String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Jiaolianxinxi.selectePicList");
	...
	public ModelAndView doDelePic(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ...
	    try {
		    ...
		    //向数据库提交删除信息
		    sqlMap.delete("Jiaolianxinxi.doDelePic", where);
