package  app.jianshenqixieguanli.jianshenqixiexinxi;
...
public class Jianshenqixiexinxi extends BaseFormController {
    //业务处理方法
    public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            if (flag != null && flag.equals("getJsonStore")) {
                ...
                return toFinish("/WEB-ROOT/app/jianshenqixieguanli/jianshenqixiexinxi/jianshenqixiexinxi.jsp", resultMap, request,  response);
            }
            //提交数据库
    ...
    public ModelAndView doGetJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            String qixiemingchenSearch = request.getParameter("qixiemingchenSearch"); 
 if( qixiemingchenSearch != null && !qixiemingchenSearch.equals("") ){  
 	 where.put("qixiemingchenSearch", qixiemingchenSearch);  
 } 
String caigoushijianSearch = request.getParameter("caigoushijianSearch"); 
 if( caigoushijianSearch != null && !caigoushijianSearch.equals("") ){  
 	 where.put("caigoushijianSearch", caigoushijianSearch);  
 } 
String jiageSearch = request.getParameter("jiageSearch"); 
 if( jiageSearch != null && !jiageSearch.equals("") ){  
 	 where.put("jiageSearch", jiageSearch);  
 } 
String gongnenmiaoshuSearch = request.getParameter("gongnenmiaoshuSearch"); 
 if( gongnenmiaoshuSearch != null && !gongnenmiaoshuSearch.equals("") ){  
 	 where.put("gongnenmiaoshuSearch", gongnenmiaoshuSearch);  
 } 
...
if( !SysInfo.getLoginUserType(request, response).equals("1") ){ 
	   where.put("dataRight",  "  a.operatorId='"+SysInfo.getLoginUserId(request, response)+"'  " );
            ...
            //根据查询条件获取数据
            String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Jianshenqixiexinxi.selecteList");
    ...
    public ModelAndView doAdd(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库新增数据
            sqlMap.insert("Jianshenqixiexinxi.insertObj", where);
    ...
    public ModelAndView doAddWithAttach(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if (item.isFormField()) {
					if (i == 0) {
	whereMap.put("qixiemingchen", item.getString("gb2312"));
}
else if (i == 1) {
	whereMap.put("caigoushijian", item.getString("gb2312"));
}
else if (i == 2) {
	whereMap.put("jiage", item.getString("gb2312"));
}
else if (i == 3) {
	whereMap.put("gongnenmiaoshu", item.getString("gb2312"));
}
else if (i == 4) {
 ...
 whereMap.put("erjiguanlianzd",  request.getParameter("erjiguanlianzd")  );
            ...
            //新增数据
		    sqlMap.insert("Jianshenqixiexinxi.insertObj", whereMap);
    ...
    public ModelAndView doUpdate(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交修改数据
            sqlMap.update("Jianshenqixiexinxi.updateObj", where);
	...
	whereMap.put("detail", item.getString("gb2312"));
			...
			//向数据库提交修改信息
		    sqlMap.update("Jianshenqixiexinxi.updateObj", whereMap);
    ...
    public ModelAndView doDeleOrUnDele(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交删除数据
            sqlMap.delete("Jianshenqixiexinxi.doDeleOrUnDele", where);
     ...
     public ModelAndView doShenPi(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            //向数据库提交审批信息
            sqlMap.delete("Jianshenqixiexinxi.doShenPi", where);
...
if( !SysInfo.getLoginUserType(request, response).equals("1") ){ 
    ...
    public static ModelAndView doExportExcel(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			// 根据查询条件得到结果集
			HashMap<String, Object> where = new HashMap<String, Object>();
			List<Object> listInfo = sqlMap.queryForList("Jianshenqixiexinxi.selecteList", where);
	...
	public static WritableSheet refreashSheet(List<Object> al, WritableSheet ws) throws Exception {
       ...
       //样式5
		...
		// 设置表头(列号,行号 ,内容 )
		 Label label_0 = new Label(1, 3, "器械名称", wcfFC5);  
 ws.addCell(label_0); 
 Label label_1 = new Label(2, 3, "采购时间", wcfFC5);  
 ws.addCell(label_1); 
 Label label_2 = new Label(3, 3, "价格", wcfFC5);  
 ws.addCell(label_2); 
 Label label_3 = new Label(4, 3, "功能描述", wcfFC5);  
 ws.addCell(label_3); 
 ...
 ws.addCell(label_6); 
		...
		//设置表体
		for (int i = 0; i < al.size(); i++) { String qixiemingchen = ((HashMap) (al.get(i))).get("qixiemingchen").toString();  
 label_0 = new Label(1, i + 4, qixiemingchen, wcfF4); 
 ws.addCell(label_0); 
 String caigoushijian = ((HashMap) (al.get(i))).get("caigoushijian").toString();  
 label_1 = new Label(2, i + 4, caigoushijian, wcfF4); 
 ws.addCell(label_1); 
 String jiage = ((HashMap) (al.get(i))).get("jiage").toString();  
 label_2 = new Label(3, i + 4, jiage, wcfF4); 
 ws.addCell(label_2); 
 String gongnenmiaoshu = ((HashMap) (al.get(i))).get("gongnenmiaoshu").toString();  
 label_3 = new Label(4, i + 4, gongnenmiaoshu, wcfF4); 
 ws.addCell(label_3); 
 ...
 ws.addCell(label_6); 
	...
	public ModelAndView doAddPicSubmit(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//查询图片信息
			List list = sqlMap.queryForList("Jianshenqixiexinxi.getTuPianIndex", whereMap);
			if( list != null && list.size() > 0 ){
				if( list.get(0) != null ){
			...
			if( tuPianIndex.equals("") ){
				...
				sqlMap.update("Jianshenqixiexinxi.updateTuPianIndex", whereMap);
				...
				if ( !item.isFormField()) {
				   //获取文件名称
					...
					if( ! new File( uploadPath ).exists() ){
						whereMap.put("operatorId", SysInfo.getLoginUserId(request, response));
						whereMap.put("tuPianIndex", tuPianIndex);
						sqlMap.insert("Jianshenqixiexinxi.insertTuPian", whereMap);
					}
	...
	public ModelAndView doGetPicJsonStore(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//得到查询图片结果信息
			String json = IbatisUtil.queryForPage(sqlMap, request, response, where, "Jianshenqixiexinxi.selectePicList");
	...
	public ModelAndView doDelePic(SqlMapClient sqlMap, HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ...
	    try {
		    ...
		    //向数据库提交删除信息
		    sqlMap.delete("Jianshenqixiexinxi.doDelePic", where);
