...
ManageObj.initDataGridPan = function() {
	...
	ManageObj.dataStore = new Ext.data.JsonStore({
		...
		remoteSort : true,
		fields : [ 'kaleixing', 'id', 'itime', 'detail', 'deleteFlag', 'erjiguanlianzd' ],
		proxy : new Ext.data.HttpProxy({
			url : '/WEB-ROOT/app/jianshenjibenshezhi/huiyuankaleixing.do',
			method : 'POST'
		}),
		baseParams : {
			flag : 'getJsonStore',
	...
	ManageObj.dataGridPan = new Ext.grid.GridPanel({
		 ...
		 */
		{
			...
			header : '<font color="blue">卡类型</font>',
			width : 60,
			align : 'center',
			sortable : true,
			dataIndex : 'kaleixing'
...
ManageObj.doSearch = function() {
	...
	ManageObj.dataStore.baseParams.kaleixingSearch = document.all.kaleixingSearch.value;
...
ManageObj.getOperationFormPan = function(rowIndex, flag) {
	ManageObj.operationFormPan = new Ext.FormPanel({
		...
		items : [ {
			...
			items : [ {
				...
				items : [ {
					xtype : 'textfield',
					fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;卡类型',
					name : 'kaleixing',
					id : 'kaleixing',
					anchor : '95%',
					allowBlank : false
	...
	if (rowIndex != null) {
		ManageObj.operationFormPan.findById('kaleixing').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('kaleixing');
...
ManageObj.doExportExcel = function() {
	var url = "/WEB-ROOT/app/jianshenjibenshezhi/huiyuankaleixing.do?flag=doExportExcel";
	window.open(url);
};
