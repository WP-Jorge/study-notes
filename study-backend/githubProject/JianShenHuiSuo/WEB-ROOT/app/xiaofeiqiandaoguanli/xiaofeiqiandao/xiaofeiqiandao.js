var ManageObj = function() {
	...
	ManageObj.huiyuanzhanghaoGuanLianStore = new Ext.data.JsonStore({
		url : '/WEB-ROOT/app/xiaofeiqiandaoguanli/xiaofeiqiandao.do',
		baseParams : {
			flag : 'doGuanLian',
			guanLianBiao : 'zhucehuiyuan',
			guanLianZiDuan : 'yonghuzhanghao'
		},
		root : 'root',
		totalProperty : 'totalProperty',
		fields : [ 'yonghuzhanghao' ]
	});
				...
				items : [
					{
						...
						xtype : 'textfield',
						fieldLabel : '会员帐号',
						name : 'huiyuanzhanghaoSearch',
						id : 'huiyuanzhanghaoSearch',
						anchor : '100%'
					} ]
			}
				,
					...
					items : [
						{
							xtype : 'datefield',
							fieldLabel : '消费日期',
							name : 'xiaofeiriqiSearch',
							id : 'xiaofeiriqiSearch',
							anchor : '100%'
						} ]
				}
				,
		...
		buttons : [ {
			...
			handler : function() {
				ManageObj.chartWindow = new Ext.Window({
					...
					resizable : false,
					layout : 'fit',
					html:"<iframe scrolling=no src='/WEB-ROOT/app/xiaofeiqiandaoguanli/xiaofeiqiandao.do?flag=doChart' frameborder='0' width='100%' height='400' ></iframe>"
				});
...
ManageObj.initDataGridPan = function() {
	...
	ManageObj.dataStore = new Ext.data.JsonStore({
		...
		totalProperty : 'totalProperty',
		remoteSort : true,
		fields : [ 'huiyuanzhanghao', 'xiaofeiriqi', 'shouliren', 'shoulishijian', 'id', 'itime', 'detail', 'deleteFlag', 'erjiguanlianzd' ],
	...
	ManageObj.dataGridPan = new Ext.grid.GridPanel({
		...
		columns : [
			...
			*/
			...
			, {
				header : '<font color="blue">会员帐号</font>',
				width : 60,
				align : 'center',
				sortable : true,
				dataIndex : 'huiyuanzhanghao'
			}
			, {
				header : '<font color="blue">消费日期</font>',
				width : 60,
				align : 'center',
				sortable : true,
				dataIndex : 'xiaofeiriqi'
			}
			, {
...
ManageObj.doSearch = function() {
	...
	ManageObj.dataStore.baseParams.huiyuanzhanghaoSearch = document.all.huiyuanzhanghaoSearch.value;
	ManageObj.dataStore.baseParams.xiaofeiriqiSearch = document.all.xiaofeiriqiSearch.value;
...
ManageObj.getOperationFormPan = function(rowIndex, flag) {
	ManageObj.operationFormPan = new Ext.FormPanel({
		...
		items : [ {
			...
			items : [ {
				...
				items : [ {
					xtype : 'combo',
					fieldLabel : '<font color="red">*</font>&nbsp;会员帐号',
					store : ManageObj.huiyuanzhanghaoGuanLianStore,
					valueField : 'yonghuzhanghao',
					displayField : 'yonghuzhanghao',
					mode : 'remote',
					forceSelection : false,
					hiddenName : 'huiyuanzhanghaoHidd',
					triggerAction : 'all',
					editable : true,
				...
				, {
					...
					items : [ {
						xtype : 'datefield',
						format : 'Y-m-d',
						fieldLabel : '<font color="red">*</font>&nbsp;&nbsp;消费日期',
						name : 'xiaofeiriqi',
						id : 'xiaofeiriqi',
						anchor : '95%',
						readOnly : true,
	...
	if (rowIndex != null) {
		ManageObj.operationFormPan.findById('huiyuanzhanghao').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('huiyuanzhanghao');
		ManageObj.operationFormPan.findById('xiaofeiriqi').value = ManageObj.dataGridPan.getStore().getAt(rowIndex).get('xiaofeiriqi');
...
ManageObj.doExportExcel = function() {
	var url = "/WEB-ROOT/app/xiaofeiqiandaoguanli/xiaofeiqiandao.do?flag=doExportExcel";
	window.open(url);
};
