...
ManageObj.setUserMenu = function(id) {
	var userMenuWindow = Ext.extend(Ext.Window, {
		...
		resizable : false,
		title : '用户菜单授权管理',
		html : "<iframe id='userMenuFrame' name='userMenuFrame' src='" + GLOBAL_APP_NAME + "/WEB-ROOT/app/sysManage/userMenuManage.do?userID=" + id + "' frameborder='0' width='100%' style='height:100%'></iframe>",
...
ManageObj.updateStatus = function(id, status) {
	Ext.MessageBox.confirm('注意！', '您确认需要修改状态吗？', function(info) {
		if (info == "yes") {
			Ext.Ajax.request({
				waitMsg : '数据提交中......',
				url : '/WEB-ROOT/app/sysManage/systemUserManage.do',
