...
public class MyInterceptor implements HandlerInterceptor {
	 ...
	 * false:请求没有通过拦截器的验证，请求到达拦 截器就截止了，请求没有被处理
	 * 拦截器的MyInterceptor的preHandle()
	 * 特点：
	 ...
	 * 2.能够获取到处理器方法的返回值Mode lAndView,可以修改ModelAndVi ew中的
	 * 数据和视图，可以影响到最后的执行结果。
	 * 3.主要是对原来的执行结果做二次修正，
	 */
	 ...
	 * 特点:
	 * 1.在请求处理完成后执行的。框架中规定是当你的视图处理完成后，对视图执行 了forward。就认为请求处理完成。
