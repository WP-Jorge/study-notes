...
<html lang="zh-CN">
  ...
  <%
      ...
      int pstatus=Integer.parseInt(request.getParameter("p_status").toString().trim());
      int expert_id=new UserDao().getUserByMail((String)session.getAttribute("umail")).getUid();
      int pageSize=10;//每页显示条数
      ProjectDao pd=new ProjectDao();
  	int totalCount=pd.getCountByStatusandExpert(pstatus,expert_id);//p_status项mu 总共条数
            ...
            <table class="table table-hover">
              ...
              <%
              	List<Project> plist = new ArrayList<Project>();
              		plist=pd.expertlistByPage(pstatus,np,expert_id);//查询1状态的项目
  			...
  			<tr>
  				...
  				<td><a href="checkapplydetail.jsp?pid=<%=pid %>&&pstatus=<%=pstatus %>">查看编辑</a></td>
  			</tr>
  			<%} %>
  			</tbody>
  			</table>
			<center>
  			<form action="expertcheck.jsp" method="post" name="form2">
 <table>
 <tr>  
 <td>共<%out.print(totalCount);%>条记录,共<%out.print(totalpages);%> 页，当前是第<%out.print(np);%>  页</td> 
 <td> 
     <a href="expertcheck.jsp?page=<%=np - 1%>&&p_status=<%=pstatus %>&&expert_id=<%=expert_id %>">上一页&nbsp; </a> 
     <a href="expertcheck.jsp?page=<%=np + 1%>&&p_status=<%=pstatus %>&&expert_id=<%=expert_id %>">下一页&nbsp;  </a> 
     <a href="expertcheck.jsp?page=1&&p_status=<%=pstatus %>&&expert_id=<%=expert_id %>">首页 &nbsp;</a> 
     <a href="expertcheck.jsp?page=<%=totalpages%>&&p_status=<%=pstatus %>&&expert_id=<%=expert_id %>">尾页  </a> 
