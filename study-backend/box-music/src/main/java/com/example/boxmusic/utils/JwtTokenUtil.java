...
public class JwtTokenUtil {
     ...
     * @return 是否有效
     */
    public Boolean validateToken(String token, MyUserDetails userDetails) throws Exception {
    ...
    private String generateToken(Map<String, Object> claims) {
        ...
        // String secret_key = System.currentTimeMillis() + UUID.randomUUID().toString();
    
        String token = Jwts.builder().setClaims(claims).setExpiration(expirationDate).signWith(SignatureAlgorithm.HS512, SECRET_KEY).compact();
    ...
    private Claims getClaimsFromToken(String token) throws Exception {
        Claims claims = null;
        try {
            // String secret_key = (String) redisTemplate.opsForValue().get(token);
