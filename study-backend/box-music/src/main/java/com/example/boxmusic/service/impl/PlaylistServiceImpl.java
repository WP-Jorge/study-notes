...
public class PlaylistServiceImpl extends ServiceImpl<PlaylistMapper, Playlist> implements PlaylistService {
	...
	@Override
	public R addPlaylistToCollection(String headerToken, Long playlistId) {
		if ("".equals(headerToken) || headerToken == null) {
			return R.error("token异常");
		}
		// postMan测试时，自动假如的前缀，要去掉。
		...
		UserVO userInfo = userService.getUserInfoWithUsername(username);
		try {
			Integer i = baseMapper.addPlaylistToCollection(userInfo.getUserId(), playlistId);
			if (i > 0) {
				return R.success("收藏歌单成功");
			}
			return R.error("收藏歌单失败");
			// redisTemplate.delete("com.example.boxmusic.mapper.MusicMapper");
		} catch (DuplicateKeyException e) {
			throw new DuplicateKeyException("收藏失败，该歌单已被收藏");
		} catch (Exception e) {
			throw new RuntimeException(e.getMessage());
		}
	}
	...
	@Override
	public R addMusicToPlaylist(AddMusicPlaylistDTO addMusicPlaylistDTO) {
		try {
			Integer integer = baseMapper.addMusicToPlaylist((addMusicPlaylistDTO));
			if (integer > 0) {
				return R.success("添加至歌单成功");
			}
			return R.error("添加至歌单失败");
		} catch (DuplicateKeyException e) {
			throw new DuplicateKeyException("收藏失败，该歌曲已在歌单中");
		} catch (Exception e) {
			throw new RuntimeException(e.getMessage());
		}
	...
	@Override
	public R deleteMusicFromPlaylist(AddMusicPlaylistDTO addMusicPlaylistDTO) {
		Integer integer = baseMapper.deleteMusicFromPlaylist((addMusicPlaylistDTO));
		if (integer > 0) {
			return R.success("删除成功");
		}
		return R.error("删除失败");
